...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_order_products` AS select `order`.`idOrder` AS `idOrder`,`order`.`createDate` AS `createDate`,`order`.`paid` AS `paid`,`order`.`send` AS `send`,`view_product_brand`.`nameProduct` AS `nameProduct`,`view_product_brand`.`nameBrand` AS `nameBrand`,`view_product_brand`.`price` AS `price`,`productorder`.`amount` AS `amount` from ((`order` join `productorder` on((`order`.`idOrder` = `productorder`.`Order_idOrder`))) join `view_product_brand` on((`productorder`.`Product_idProduct` = `view_product_brand`.`idProduct`))) order by `productorder`.`Order_idOrder` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2023-02-03 19:17:46
